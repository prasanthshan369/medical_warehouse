    ...
    return (
        <View className="flex-1 bg-[#F9F9F9]">
            ...
            <View className="px-5 py-5 mt-6 flex-row items-center bg-white border-b border-[#EEEEEE]">
                ...
                <Text className="text-[18px] font-inter-bold" style={{ color: colors.text.DEFAULT }}>Order Summary</Text>
            </View>
            ...
            <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 150 }}>
                ...
                <View className="m-5 bg-white p-6 rounded-[16px] border border-[#EEEEEE]">
                    <Text className="text-[20px] font-inter-bold mb-4" style={{ color: colors.text.DEFAULT }}>
                        #{orderId}
                    </Text>
                    <View className="flex-row items-center">
                        ...
                        <View>
                            <Text className="text-[12px] font-inter-medium mb-0.5" style={{ color: colors.text.secondary }}>Name</Text>
                            <Text className="text-[16px] font-inter-semibold" style={{ color: colors.text.DEFAULT }}>{order?.customerName || 'N/A'}</Text>
                        </View>
                    </View>
                </View>

                {/* Section Title */}
                <Text className="px-5 text-[14px] font-inter-medium mb-3" style={{ color: colors.text.secondary }}>Ordered Items</Text>

                {/* Items List */}
                <View className="px-5 space-y-4">
                    {order?.pickingItems?.map((item) => (
                        <View key={item.id} className="bg-white p-6 rounded-[24px] border border-[#EEEEEE] mb-4">
                            <Text style={{ color: colors.text.DEFAULT }} className="font-inter-bold text-[18px] mb-1">{item.name}</Text>
                            <Text style={{ color: colors.text.secondary }} className="text-[14px] mb-4 font-inter">{item.manufacturer}</Text>
                            ...
                            <View className="flex-row items-center mb-6">
                                ...
                                >
                                    <Text style={{ color: colors.text.secondary }} className="text-[12px] font-inter-semibold">Batch No: {item.batchNo}</Text>
                                </View>
                                <View
                                    style={{ backgroundColor: colors.surface.gray }}
                                    className="px-3 py-1.5 rounded-lg"
                                >
                                    <Text style={{ color: colors.text.secondary }} className="text-[12px] font-inter-semibold">EXP {item.expiryDate}</Text>
                                </View>
                            </View>

                            <View className="flex-row items-baseline mb-2">
                                <Text
                                    className="text-[47px] font-inter-bold"
                                    style={{ color: colors.brand.primary, lineHeight: 52 }}
                                >
                                    {item.requiredQty}
                                </Text>
                                <Text style={{ color: colors.text.DEFAULT }} className="text-[14px] font-inter-medium ml-2">
                                    Units Required
                                </Text>
                            </View>
                            ...
                            {item.description && (
                                <View className="flex-row justify-end">
                                    <Text style={{ color: colors.text.DEFAULT }} className="text-[11px] font-inter-medium">
                                        {item.description}
                                    </Text>
                                </View>
                            )}
